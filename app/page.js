  ...
  return (
    <main>
      <section className="bg-[#254f1a] min-h-[100vh] grid grid-cols-2">
        <div className="flex justify-center flex-col ml-[10vw] gap-3 mt-12">
          ...
          <div className="input flex gap-2">
            <input value={text} onChange={(e)=> setText(e.target.value)} className="px-2 py-2 focus:outline-green-800 rounded-m bg-white text-black rounded-md" type="text" placeholder="Enter your Handle" />
            <button onClick={()=>createTree()} className="bg-pink-400 rounded-full px-4 py-4 font-semibold">Claim your Bittree</button>
          </div>
           <Link href={"/generate"}><button className="bg-red-500 rounded-full px-4 py-4 font-semibold w-full">Start Now</button></Link>
        </div>
